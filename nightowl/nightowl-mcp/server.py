"""
NIGHTOWL - Unified Offensive Security MCP
Nation-state level AI-coordinated offensive platform.
...
def ai_fingerprint(responses: list) -> dict:
    """Fingerprint AI model from probe responses.
...
def jailbreak_generate(
    ...
    """Generate jailbreak attempts for AI systems.
...
def operation_start(
    ...
    """
    op = {
        "id": f"nightowl_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}",
        "target": target,
...
def list_capabilities() -> dict:
    """List all NIGHTOWL capabilities by category."""
    return {
        "recon": [
            "autonomous_recon", "dns_enum", "http_probe", "content_analyze"
        ],
...
async def run_server():
    ...
    logger.info(f"Starting NightOwl MCP server on port {port}")
