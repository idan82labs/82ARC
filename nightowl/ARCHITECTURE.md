...
Technical architecture documentation for the NIGHTOWL offensive security platform.
...
class ArtifactStorage:
    ...
    _base_path: Path  # /tmp/nightowl/artifacts/
...
- Non-root user (`nightowl`, UID 1000)
...
└─────────────────┘         └─────────────────┘
        │
        │  register_tool("nightowl", url, introspect=True)
        │
        ▼
┌─────────────────┐
│  Tool Registry  │
